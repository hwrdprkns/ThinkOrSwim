...
PLOT RATING =
IF RSI_HIGH > RSI_HIGH[13] AND ROUND(RSI_HIGH-RSI_HIGH[13],2) >= MOVEMENT AND RSI_HIGH[13] <= OVER_BOUGHT AND CLOSE > EXPAVERAGE(CLOSE,50) THEN ROUND(RSI_HIGH-RSI_HIGH[13],0)
ELSE IF RSI_LOW < RSI_LOW[13] AND ROUND(RSI_LOW-RSI_LOW[13],2) <= -MOVEMENT AND RSI_LOW[13] >= OVER_SOLD AND CLOSE < EXPAVERAGE(CLOSE,50) THEN ROUND(RSI_LOW-RSI_LOW[13],0)
ELSE 0;
