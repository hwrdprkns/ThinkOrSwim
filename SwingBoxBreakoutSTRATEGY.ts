...
DEF MCO = (MCCLELLANOSCILLATOR()>0 OR (MCCLELLANOSCILLATOR() > ((MCCLELLANOSCILLATOR()[1]+MCCLELLANOSCILLATOR()[2])/2) AND CORRELATION(CLOSE, CLOSE("SPY"), 10)>0)) OR CORRELATION(CLOSE, CLOSE("SPY"), 10) <= -0.50;

DEF CHANGE = CLOSE > CLOSE[1];

DEF BUYSIGNAL = VOLUMEAVG(LENGTH = AVGVOL) > VOLUMEAVG(LENGTH = AVGVOL).VOLAVG AND CHANGE IS TRUE;

DEF ENTRY = BUYSIGNAL IS TRUE AND RSI IS TRUE AND STOCH IS TRUE AND MCO IS TRUE;
